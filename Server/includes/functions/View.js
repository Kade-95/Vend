...
class PostHandler {
    ...
    createTenant(req, res, data) {
        data.tenant = `${data.tenant}#onCrater`;
        this.ifNotExist(req, { collection: 'tenants', query: { name: data.tenant }, check: [{ name: data.tenant }], action: 'insert' }).then(inserted => {
            if (inserted == 1) {
                global.sessions[req.sessionId].db.setName(data.tenant);//set database name
                global.sessions[req.sessionId].db.erase().then(erased => {//erase database
                    if (erased) {
                        this.setUpTenant(req, data, user => {
                            ...
                            global.sessions[req.sessionId].set({ user: ObjectId(user._id).toString(), active: true, account: data.tenant });
    ...
    fetchCloudApp(req, res, data) {
        if (!kerds.isset(data._id) || data._id == '') {
            this.respond(req, res, null);
            return;
        }

        this.find({ collection: 'apps', query: { _id: new ObjectID(data._id), page: data.page } }).then(result => {
            this.respond(req, res, result);
    ...
    putCloudApp(req, res, data) {
        let attributes = JSON.parse(data.attributes);
        if (!kerds.isset(attributes._id) || attributes._id == '') {
            delete attributes._id;
            this.insert({ collection: 'apps', query: attributes, getInserted: true }).then(result => {
            ...
            this.save({ collection: 'apps', query: attributes, check: { _id: attributes._id, page: attributes.page } }).then(result => {
    ...
    deleteCloudApp(req, res, data) {
        if (!kerds.isset(data._id) || data._id == '') {
            this.respond(req, res, 'null');
            return;
        }

        this.delete({ collection: 'apps', query: { _id: new ObjectID(data._id) } }).then(result => {
            this.respond(req, res, result);
...
module.exports = { PostHandler };class View  {
